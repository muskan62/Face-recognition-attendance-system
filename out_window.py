    ...
    def face_rec_(self, frame, encode_list_known, class_names):
        ...
        def mark_attendance(name):
            ...
            if self.ClockInButton.isChecked():
                self.ClockInButton.setEnable(False)
                ...
                with open('Attendance.csv', 'a') as f:
                    if (name != 'unknown'):
                        ...
                        if buttonReply == QMessageBox.Yes:
                            ...
                            self.Elapse.List(name)
                            self.TimeList2.append(datetime.datetime.now())
                            CheckInTime = self.TimeList1[-1]
                            CheckOutTime = self.TimeList2[-1]
                            self.ElapseHours = (CheckOutTime - CheckInTime)

                            self.HoursLabel.setText("{:.0f}".format(abs(self.ElapseHours.total_total_seconds()/60**2)) + 'n')
                            self.MinLabel.setText("{:.0f}".format(abs(self.ElapseHours.total_total_seconds()/60)%60) + 'n')
            ...
            for row in csv_reader:
                for field in row:
                    if field in row:
                        if field == 'Clock In':
                            if row[0] == name:
                                Time1 =(datetime.datetime.strptime(row[:],'%y/%m/%d %H:%M:%S'))
                                self.TimeList1.append(Time1)
                        if field == 'Clock Out':
                            if row[0] == name:
                                Time2 =(datetime.datetime.strptime(row[:],'%y/%m/%d %H:%M:%S'))
